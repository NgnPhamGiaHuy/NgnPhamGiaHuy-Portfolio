...
const SideBar: React.FC<SideBarProps> = React.memo(({ list }) => {
    const socialLinks = useMemo(() =>
        list.map((item: SocialProfilesProps, index:number) => (
    ...
    return (
        <div className={"max-w-20 w-full h-screen pb-16 top-0 left-0 max-xl:hidden flex flex-col justify-between border-r border-solid border-primary-2 fixed"}>
            <Link href={"/"}>
