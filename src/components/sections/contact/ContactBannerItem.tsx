...
const ContactBannerItem : React.FC<{ item: ContactItemProps, isLastIndex: boolean }> = ({ item, isLastIndex }) => {
    return (
        <li className={clsx({ "mb-6": isLastIndex }, "pb-6 relative before:w-50 before:h-[1px] before:bottom-0 before:left-0 before:absolute before:bg-primary-2/20")}>
            ...
            <div className={"flex flex-row items-center gap-4"}>
                ...
                <div>
                    ...
                    <Link href={item.href} legacyBehavior>
                        <span className={"text-sm text-text font-normal cursor-pointer leading-normal transition-all break-all overflow-hidden whitespace-normal duration-300 hover:text-primary-1"}>
                            { item.value }
                        </span>
                    </Link>
                </div>
